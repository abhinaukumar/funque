from ..tools.misc import unroll_dict_of_lists
import numpy as np
...
class RandomSplitModelCrossValidation(ModelCrossValidation):

    @classmethod
    def run_random_cross_validation(cls,
                                    train_test_model_class,
                                    model_param,
                                    results_or_df,
                                    splits,
        ...
        :param model_param:
        :param results_or_df: list of BasicResult, or pandas.DataFrame
        :param splits: Number of random splits to test on
        :param test_size: Fraction of test set
        :param kfold: if it is an integer, it is the number of folds; if it is
        list of indices, then each list contains row indices of the dataframe
        selected as one fold
        :return: output
        ...
        assert isinstance(splits, (int, long)), 'Number of splits must be an integer'
        assert isinstance(test_size, float) and test_size > 0 and test_size < 1, 'test_size must be a float value strictly between 0 and 1.'
        ...
        dataframe_size = len(results_or_df)
        test_df_size = int(test_size*dataframe_size)
        for split in range(splits):
            ...
            inds = np.random.permutation(dataframe_size)
            test_index_range = inds[:test_df_size]
            train_index_range = inds[test_df_size:]
            output = cls.run_cross_validation(train_test_model_class,
                                              model_param,
                                              results_or_df,
                                              train_index_range,
