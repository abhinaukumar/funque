...
FUNQUE_LIB_FOLDER = os.path.dirname(os.path.abspath(__file__))


# Assuming FUNQUE source checkout, path to top checked out folder
FUNQUE_PROJECT = os.path.abspath(os.path.join(FUNQUE_LIB_FOLDER, '../..',))
...
def project_path(relative_path):
    path = os.path.join(FUNQUE_PROJECT, relative_path)
    return path
...
class ExternalProgram(object):
    ...
    try:
        ...
        vmaf_feature = config.FunqueExternalConfig.vmaf_path()
        vmafossexec = config.FunqueExternalConfig.vmafossexec_path()
        vmafexec = config.FunqueExternalConfig.vmafexec_path()
    except ImportError:
        vmaf_feature = None
        vmafossexec = None
        vmafexec = None
