...
from . import plt
from .core.cross_validation import ModelCrossValidation, RandomSplitModelCrossValidation
from .core.feature_assembler import FeatureAssembler
from .core.quality_runner import FunqueQualityRunner
from .core.result_store import FileSystemResultStore
from .tools.misc import indices, get_stdout_logger, import_python_file, close_logger, get_file_name_without_extension
from .config import FunqueConfig, DisplayConfig
from .core.asset import Asset
from .core.train_test_model import TrainTestModel, RegressorMixin, ClassifierMixin
...
def read_dataset(dataset, **kwargs):
    ...
    workdir_root = kwargs['workdir_root'] if 'workdir_root' in kwargs else FunqueConfig.workdir_path()
...
def train_test_on_dataset(train_dataset, test_dataset,
    ...
    if processes is not None:
        assert parallelize is True, 'if processes is not None, parallelize must be True'
    optional_dict = kwargs.get('optional_dict', None)
    optional_dict2 = kwargs.get('optional_dict2', None)
    ...
    # FunqueQualityRunner can read and process
    model.append_info('feature_dict', feature_param.feature_dict)
    model.append_info('feature_optional_dict', optional_dict)
    model.append_info('feature_optional_dict2', optional_dict2)

    if 'score_clip' in model_param_dict:
        FunqueQualityRunner.set_clip_score(model, model_param_dict['score_clip'])
    if 'score_transform' in model_param_dict:
        FunqueQualityRunner.set_transform_score(model, model_param_dict['score_transform'])

    train_ys_pred = FunqueQualityRunner.predict_with_model(model, train_xs, **kwargs)['ys_pred']
    ...
    if test_dataset is None:
        ...
        test_ys_pred = FunqueQualityRunner.predict_with_model(model, test_xs, **kwargs)['ys_pred']
...
def random_cv_on_dataset(dataset, feature_param, model_param, ax, result_store,
                         logger=None, aggregate_method=np.mean, **kwargs):

    assets = read_dataset(dataset)

    processes = kwargs['processes'] if 'processes' in kwargs else None
    parallelize = kwargs['parallelize'] if 'parallelize' in kwargs else True
    splits = kwargs['splits'] if 'splits' in kwargs else 5000
    ...
    fassembler = FeatureAssembler(
        ...
        optional_dict=kwargs.get('optional_dict', None),
        optional_dict2=kwargs.get('optional_dict2', None),
        parallelize=parallelize, fifo_mode=True,
        processes=processes
        # parallelize=False, fifo_mode=False, # VQM
    )
    fassembler.run()
    ...
    model_class = TrainTestModel.find_subclass(model_param.model_type)
    # run random cv
    cv_output = RandomSplitModelCrossValidation.run_random_cross_validation(
        model_class,
        model_param.model_param_dict,
        results,
        splits=splits,
...
def run__cv(train_dataset_filepath,
                test_dataset_filepath,
                param_filepath,
                output_model_filepath=None,
                **kwargs):

    result_store_dir = kwargs['result_store_dir'] if 'result_store_dir' in kwargs else FunqueConfig.file_result_store_path()
...
def run__kfold_cv(dataset_filepath,
                      contentid_groups,
                      param_filepath,
                      aggregate_method,
                      result_store_dir=FunqueConfig.file_result_store_path(),
                      ):
...
def run__random_cv(dataset_filepath,
    ...
    assets, cv_output = random_cv_on_dataset(dataset, feature_param, model_param, ax, result_store,
                                             logger, aggregate_method, **kwargs)
...
def run__cv_from_raw(train_dataset_raw_filepath, test_dataset_raw_filepath,
    ...
    workspace_path = kwargs['workspace_path'] if 'workspace_path' in kwargs else FunqueConfig.workspace_path()
...
def run__random_cv_from_raw(train_dataset_raw_filepath, test_dataset_raw_filepath,
                                param_filepath, output_model_filepath, **kwargs):
    if 'train_quality_wh' in kwargs and kwargs['train_quality_wh'] is not None:
        train_quality_width, train_quality_height = kwargs['train_quality_wh']
    else:
